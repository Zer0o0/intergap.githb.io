...
蛋白质-DNA相互作用和表观遗传标记的全基因组图谱对于全面理解转录调控至关重要。转录因子、核心转录机制和其他 DNA 结合蛋白结合位点的精确图谱对于破译构成各种生物过程的基因调控网络至关重要。核小体定位与 DNA 和组蛋白的动态修饰相结合，在基因调控中发挥关键作用，并指导发育和分化。染色质状态可以通过改变 DNA 的包装直接影响转录，以允许或阻止 DNA 结合蛋白的访问，或者它们可以修饰核小体表面以增强或阻碍效应蛋白复合物的募集。最近的进展表明，染色质和转录之间的这种相互作用是动态的，并且比以前认识到的更复杂，并且人们越来越认识到，可能需要对多种细胞类型和阶段的表观基因组进行系统分析，以了解发育过程和疾病状态。
染色质免疫沉淀后测序 (ChIP-seq, Chromatin immunoprecipitation followed by sequencing) 是一种对 DNA 结合蛋白、组蛋白修饰或核小体进行全基因组分析的技术。 由于新一代测序技术的巨大进步，ChIP-seq 提供了更高的分辨率、更低的噪音和更大的覆盖范围。ChIP-seq 实验产生大量数据，有效的计算分析对于揭示生物学机制至关重要。
